 ...
 * 租借工具
 */
@WebServlet("/borrower/rent-tool")
public class RentToolServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String toolIdParam = request.getParameter("toolId");

        try {

            Integer toolId = Integer.parseInt(toolIdParam);
            borrowerService.rentTool(currentUser.getUserId(), toolId);
        } catch (Exception e) {
            System.err.println("租借工具异常: " + e.getMessage());
            request.setAttribute("error", "租借申请失败，请稍后重试，错误信息: " + e.getMessage());
        }
